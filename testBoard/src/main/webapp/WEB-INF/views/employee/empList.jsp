...
<c:set var="totCnt" value="${insertMap.totCnt}"/>
<c:set var="pagination" value="${insertMap.pagination}"/>
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
<title>사원 검색 목록</title>
	...
	$(function(){
		...
		$("#search").click(function(){
			...
			if($("#searchWord").val() == null || $("#searchWord").val() == ""){
				...
				f.method="get";
				f.action="${pageContext.request.contextPath}/employee/empList/1";
				f.submit();
			}
		});	
		// 등록
...
*/
	...
	var numbering = 6;
	function nextPaging(){
		if(numbering <= totalPage){
			location.href="${pageContext.request.contextPath}/employee/search/${selectGroup}/"+numbering+"?searchWord=${searchWord}";
			numbering += 5;
		}else{
			location.href="${pageContext.request.contextPath}/employee/search/${selectGroup}/"+totalPage+"?searchWord=${searchWord}";
		}
	}
	function prePaging(){
		if(page >= 6){
			...
			location.href="${pageContext.request.contextPath}/employee/search/${selectGroup}/1?searchWord=${searchWord}";
		}
				...
				<table width="640" border="0" cellspacing="0" cellpadding="0">
                    ...
                    <tr> 
                       ...
                       <c:forEach  var="page" items="${pageList}">
                       		<a href="${pageContext.request.contextPath}/employee/search/${selectGroup}/${page}?searchWord=${searchWord}"><c:out value="${page}"/></a> &nbsp;| &nbsp;
                       </c:forEach>
                        &nbsp;
                     	<a href="">
